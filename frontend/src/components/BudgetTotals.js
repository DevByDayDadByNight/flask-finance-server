...
import "./BudgetTotals.css"; // Import the CSS file
...
const BudgetTotals = ({ txns, lineItems, selectedBudget }) => {
  ...
  function calculateTotalForAType(type, transactions, lineItems) {
    try {
      ...
      const categoryMap = lineItems
        ...
        .reduce((acc, item) => {
          acc[item.name.toLowerCase()] = item.related_categories.map((cat) => cat.toLowerCase());
          return acc;
        }, {});
      ...
      return Object.entries(categoryMap).reduce((acc, [key, relatedCategories]) => {
        acc[key] = relatedCategories.reduce((sum, cat) => sum + (totalsNotCombined[cat] || 0), 0);
        return acc;
      }, {});
    } catch (error) {
  ...
  const spends = (spendBudgeted, spendActual) => {
    ...
    const daysPassedSince = daysPassed(new Date(budget.start_date), referenceDate) * -1;
    const daysRemaining = daysPassed(referenceDate, new Date(budget.end_date)) * -1;
    ...
    return (
      <div className="spends-block">
        <p>Days Passed: {daysPassedSince}</p>
        <p>Days Remaining: {daysRemaining}</p>
        <p>Budgeted Daily Spend: {formatMoney(spendBudgeted / (daysPassedSince + daysRemaining))}</p>
        <p>Daily Spend: {formatMoney(dailySpend)}</p>
        <p>Projected Remaining: {formatMoney(projectedRemaining)}</p>
        <p>Projected Total Spend: {formatMoney(projectedTotal)}</p>
      </div>
    );
  };
        ...
        <tbody>
          {lineItems.map((item) => (
            <tr key={item.name}>
              ...
              <td></td>
              <td>{formatMoney(totals.catTotal?.[item.name] ?? item.amount)}</td>
            </tr>
          ))}
          <tr>
            <td>Budgeted</td>
